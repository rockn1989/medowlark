...
test('запрос расценок для группы по странице туров по реке Худ' + '  должен заполнять поле реферера', function (done) {
...
test('посещение страницы "Запрос цены для группы" напрямую' + ' должен приводить к пустому полю реферера', function (done) {
